...
const TASK_DAG_REGISTRY_ABI = [
  "function submitDAG(bytes32 dagRoot, tuple(bytes32 taskId, bytes32 inputSchemaHash, bytes32 outputSchemaHash, bytes32 qualityRubricHash, bytes32[] dependsOn, uint8 nodeType, uint256 maxBudget, uint256 timeoutBlocks, address assignedAgent, uint8 status, uint256 assignedAt, uint256 completedAt)[] taskNodes) payable",
...
export class MeshClient {
  ...
  constructor(signerOrProvider: Signer | Provider, config: SDKConfig = {}) {
    ...
    this.agentRegistry = new Contract(
      finalConfig.agentRegistryAddress!,
      AGENT_REGISTRY_ABI,
      signerOrProvider,
    );
    this.taskDagRegistry = new Contract(
      finalConfig.taskDagRegistryAddress!,
      TASK_DAG_REGISTRY_ABI,
      signerOrProvider,
    );
    this.teeVerifierBridge = new Contract(
      finalConfig.teeVerifierBridgeAddress!,
      TEE_VERIFIER_BRIDGE_ABI,
      signerOrProvider,
    );
  }
  ...
  public get taskDAG() {
    return {
      submit: async (params: { nodes: NodeConfig[] }): Promise<ContractTransactionResponse> => {
        let totalBudget = 0n;
        const dagRoot = ethers.id(`dag-${Date.now()}`);
        const taskIds = params.nodes.map((_, i) => ethers.id(`${dagRoot}-node-${i}`));
        const formattedNodes = params.nodes.map((n, i) => {
          ...
          return {
            ...
            dependsOn: n.dependsOn.map((depIdx) => taskIds[depIdx]),
        ...
        return await this.taskDagRegistry.submitDAG(dagRoot, formattedNodes, {
          value: totalBudget,
        });
      },
    };
