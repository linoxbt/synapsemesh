...
const AWARD_BID_FN = parseAbiItem(
  "function awardBid(bytes32 taskId, address winnerAgent) external",
);
...
function scoreBid(bid) {
  const rep = Number(bid.reputation || 0);
...
async function settle(taskId) {
  ...
  try {
    ...
    console.error(
      `[auctioneer] awarded() read failed for ${taskId.slice(0, 10)}…: ${e.shortMessage || e.message}`,
    );
  }
  ...
  try {
    ...
    console.error(
      `[auctioneer] getBids() failed for ${taskId.slice(0, 10)}…: ${e.shortMessage || e.message}`,
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      `[auctioneer] awardBid failed for ${taskId.slice(0, 10)}…: ${e.shortMessage || e.message}`,
    );
  }
}
...
startWatching();
console.log(
  `[auctioneer] watching BidSubmitted on ${BID_ENGINE} (poll ${POLL_INTERVAL_MS}ms, window ${COLLECTION_WINDOW_MS}ms)`,
);

// Keep the process alive even if all event watchers are paused.
process.stdin.resume();
process.on("SIGINT", () => {
  console.log("\n[auctioneer] shutting down");
  process.exit(0);
});
